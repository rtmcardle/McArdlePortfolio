...
def solvefxy1():
    ...
    cur_dir = os.getcwd()
    prob_dir = os.path.join(cur_dir,'Record\\f(x,y,1)\\')
...
def solvefxy3():
    ...
    timestamp = datetime.datetime.now().strftime('%Y%m%d-%H%M%S.%f')
    cur_dir = os.getcwd()
    prob_dir = os.path.join(cur_dir,'Record\\f(x,y,3)\\')
    if not os.path.exists(prob_dir):
        os.mkdir(prob_dir)
    time_dir = os.path.join(prob_dir,f'{timestamp}\\')
    os.mkdir(time_dir)
...
def solveAckleys():
    ...
    timestamp = datetime.datetime.now().strftime('%Y%m%d-%H%M%S.%f')
    cur_dir = os.getcwd()
    prob_dir = os.path.join(cur_dir,'Record\\Ackleys\\')
    if not os.path.exists(prob_dir):
        os.mkdir(prob_dir)
    time_dir = os.path.join(prob_dir,f'{timestamp}\\')
    os.mkdir(time_dir)
...
def find_best_run(problem):
    ...
    if problem == 'f(x,y,1)':
        run_func = solvefxy1
    elif problem == 'f(x,y,3)':
        run_func = solvefxy3
    elif problem == 'Ackleys':
        run_func = solveAckleys
    ...
    timestamp = datetime.datetime.now().strftime('%Y%m%d-%H%M%S.%f')
    cur_dir = os.getcwd()
    prob_dir = os.path.join(cur_dir,'Record\\'+problem+'\\')
    if not os.path.exists(prob_dir):
        os.mkdir(prob_dir)
...
if __name__=='__main__':
    ...
    #solve1b()
    #solve2()
    find_best_run('f(x,y,1)')
    find_best_run('f(x,y,3)')
    find_best_run('Ackleys')
