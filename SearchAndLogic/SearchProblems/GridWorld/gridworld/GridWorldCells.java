...
public class GridWorldCells {
	...
	public GridWorldCells() {
    // 2 represents the goal, 1 represents the agent location
		state = new int[] { 0, 2, 0, 0, 0, 0, 1, 0, 0 };
	}

	public GridWorldCells(int[] state) {
		this.state = new int[state.length];
		System.arraycopy(state, 0, this.state, 0, state.length);
	}

	public GridWorldCells(GridWorldCells copyCells) {
		this(copyCells.getState());
	}
  ...
  //
	...
	public boolean equals(Object o) {
		...
		GridWorldCells aCells = (GridWorldCells) o;

    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j ++) {
        if (this.getValueAt(new XYLocation(i,j)) != aCells.getValueAt(new XYLocation(i,j))) {
          return false;
        }
      }
    }
