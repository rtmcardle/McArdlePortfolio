...
def main():
    ...
    #t.setpos(0,-275)
    #tf.BarnsleyLeaf(depth=10, size=400)
    #tf.saveImage('BarnsleyLeaf')

    tf.HeighwayPinwheel(depth=12, size=sizes)
    tf.saveImage('HeighwayPinwheel')

    #tf.SierpinskiDragon(depth=8, size=400)

    #tf.SierpinskiGasket(depth=8, size=400)
    #tf.saveImage('SierpinskiGasket')

    #tf.Pentadendrite(depth=6, size=500)
    #tf.saveImage('Pentadendrite')

    #tf.EisensteinBoundaryFull(depth=8)
    #tf.saveImage('EisensteinBoundary')

    #tf.KochSnowFlake(depth=6, size=600)
